...
class RealESRGAN_degradation(object):
    ...
    def degrade_process(self, img_gt, resize_bak=False):
        ...
        if updown_type == 'up':
            scale = np.random.uniform(1, self.opt['resize_range'][1])       # 1 ~ 1.2
        elif updown_type == 'down':
            scale = np.random.uniform(self.opt['resize_range'][0], 1)       # 0.3 ~ 1
        else:
            scale = 1
        ...
        if updown_type == 'up':
            scale = np.random.uniform(1, self.opt['resize_range2'][1])      # 1 ~ 1.2
        elif updown_type == 'down':
            scale = np.random.uniform(self.opt['resize_range2'][0], 1)      # 0.3 ~ 1
        else:
            scale = 1
        mode = random.choice(['area', 'bilinear', 'bicubic'])
        out = F.interpolate(
