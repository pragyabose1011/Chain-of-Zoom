...
from train_utils.realesrgan import RealESRGAN_degradation
...
class TextualInversionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            if self.args.deg_type == 'esrgan':
                output_t, img_t = self.degradation.degrade_process(np.asarray(gt_img)/255., resize_bak=True)
                output_t, img_t = output_t.squeeze(0), img_t.squeeze(0)
            elif self.args.deg_type == 'bicubic':
                # Bicubic downsampling degradation.
                new_size = (gt_img.width // self.args.scale_factor, gt_img.height // self.args.scale_factor)
                lr_img = gt_img.resize(new_size, resample=Image.BICUBIC)
                lr_img = lr_img.resize(gt_img.size, resample=Image.BICUBIC)
                to_tensor = transforms.ToTensor()
                img_t = to_tensor(lr_img)    # degraded input image
                output_t = to_tensor(gt_img) # ground truth image

            # Scale images to [-1, 1]
            img_t = F.normalize(img_t, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
            output_t = F.normalize(output_t, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])

            return img_t, output_t